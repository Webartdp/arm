        ...
        <div class="lang relative color-grey-60">
            ...
            <div class="lang--dropdown dropper bg-white shadow-primary">
                @foreach(['am', 'en', 'ru'] as $code)
                    <a href="{{ route('front.home', ['locale' => $code, 'tnum' => $tnum ?: null, 'date' => $date ?: null]) }}" class="flex-container align-middle helvetica-65 font-uppercase font-spacing-01 color-grey lang-item">{{ $code }}</a>
                @endforeach
            </div>
        </div>
    </div>
            ...
            <div class="loading-target">
                <div class="verify-card dynamic-view relative radius-12 bg-grey-5">
                    <div class="verify-card--img hide-sm show-md">
                        @if($settings->hero_image)<img src="{{ asset('storage/' . $settings->hero_image) }}" alt="">@else<img src="/static/img/certificate.svg" alt="">@endif
                    </div>
                    <div class="verify-card-heading text-center">
                        <div class="text-medium helvetica-75 text-height-200 color-grey font-bold color-grey verify-card-title">{{ $field('form_title', $d['form_title']) }}</div>
                    ...
                    <form class="verify-form row align-center" action="{{ route('front.home', ['locale' => $locale]) }}" method="GET" id="verification-form">
                        <div class="column small-14">
                            <z-cart-input id="component" element_count="4" max_lenght="4" withDatePicker="true" value="{{ $tnum }}" validationMessage="Invalid input" showError="false" datePickerPlaceholder="{{ $field('date_placeholder', $d['date_placeholder']) }}" datePickerActivePlaceholder="dd/mm/yyyy" datePickerDescription="dd/mm/yyyy" clear_label="Clear"></z-cart-input>
                        </div>
                ...
                <div class="text-xsmall helvetica-55 text-height-125 color-grey-60 font-spacing-01 verify-card-desc text-center show-sm hide-md">{{ $field('helper_text', $d['helper_text']) }}</div>
...
<footer class="text-center">
    @if($settings->footer_left_image || $settings->footer_right_image)
        <div class="row align-center" style="margin-bottom:16px;">
            @if($settings->footer_left_image)<div class="column small-6 large-3"><img src="{{ asset('storage/' . $settings->footer_left_image) }}" alt="" style="max-height:64px;"></div>@endif
            @if($settings->footer_right_image)<div class="column small-6 large-3"><img src="{{ asset('storage/' . $settings->footer_right_image) }}" alt="" style="max-height:64px;"></div>@endif
        </div>
    @endif
    <div class="row align-center">
...
function bindVerificationForm(){const button=document.querySelector('z-button');const component=document.getElementById('component');const form=document.getElementById('verification-form');if(!button||!component||!form||button.dataset.verifyBound==='1')return;button.dataset.verifyBound='1';button.addEventListener('click',function(event){event.preventDefault();const root=component.shadowRoot;if(!root)return;const queryString=[0,1,2,3].map(index=>root.querySelector('[name="q'+index+'"]')?.value??'').join('-');const dateInput=root.querySelector('#datepicker-component');document.getElementById('tracking_num').value=queryString;document.getElementById('issue_date').value=dateInput?.value??'';form.submit();});}
